...
namespace ft
{
	...
	class multimap
	{
		public:
			...
			typedef Pair<key_type, multimapped_type>			value_type;
		...
		public:
			...
			Pair<iterator,bool>		insert(const value_type& v)
			{
				...
				while (true)
				{
					...
					{
						if (node->elem.second > val.second)
						{
							value_type		tmp = val;

							val.second = node->elem.second;
							node->elem.second = tmp.second;
							return (insert(val));
							
						}
					}
					if (val.first == node->elem.first)
						check = val.second > node->elem.second ? false : true;
			...
			{
				multimap<key_type, multimapped_type>	tmp = x;
			...
			multimapped_type		&operator[](const key_type& k)
			{
				value_type		paire;
				MapNode			*tmp = find_node(k);
				...
				paire.first = k;
				paire.second = multimapped_type();
				insert(make_pair(k, multimapped_type()));
				return ((insert(make_pair(k, multimapped_type())).first)->second);
			}
			...
			const multimap<Key, T, Compare>& rhs)
{
	if (lhs.size() != rhs.size())
		return (false);
	for (typename multimap<Key, T, Compare>::iterator l = lhs.begin(), r = rhs.begin();
		l != lhs.end(); l++, r++)
	{
		if (*l != *r)
			return (false);
